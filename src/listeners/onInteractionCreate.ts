...
export default async function(client: ExtendedClient, interaction: Interaction) {
    if(interaction.isChatInputCommand()) {
        ...
        try {
            if(!command) { // noinspection ExceptionCaughtLocallyJS -- That's by design
    ...
    if(interaction.isModalSubmit()) {
        ...
        try {
            if(!modal) { // noinspection ExceptionCaughtLocallyJS -- That's by design
                throw "Command not found";
            }
    ...
    if(interaction.isButton()) {
        ...
        try {
            if(!button) { // noinspection ExceptionCaughtLocallyJS -- That's by design
                throw "Command not found";
            }
